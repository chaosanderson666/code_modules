...
static void _sub_bytes(uint8_t *state, const uint8_t *box)
{
        int i;
...
static void aes_inv_round(uint8_t *state, const uint8_t *inv_rk)
{
...
static void aes_final_round(uint8_t *state, const uint8_t *rk)
{
        sub_bytes(state);
        shift_rows(state);
        add_round_key(state, rk);
...
static void inv_final_round(uint8_t *state, const uint8_t *inv_rk)
{
        inv_shift_rows(state);
        inv_sub_bytes(state);
        add_round_key(state, inv_rk);
