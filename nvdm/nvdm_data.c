...
static uint16_t calculate_checksum(uint16_t checksum, const uint8_t *buffer,
                                   int32_t size)
{
...
static uint16_t calculate_data_item_checksum(data_item_header_t *header,
                int32_t pnum, int32_t position)
{
        ...
        /* checksum for data item's header
            ...
            */
        checksum = calculate_checksum(checksum, &header->pnum,
                                      DATA_ITEM_HEADER_SIZE - 1);

        /* add checksum for group name and data item name */
        offset = position;
        peb_read_data(pnum, offset, working_buffer,
                      header->group_name_size + header->data_item_name_size);
        checksum = calculate_checksum(checksum, working_buffer,
                                      header->group_name_size + header->data_item_name_size);
        ...
        if (header->value_size % NVDM_BUFFER_SIZE) {
                memset(working_buffer, 0, NVDM_BUFFER_SIZE);
                peb_read_data(pnum, offset, working_buffer,
                              header->value_size % NVDM_BUFFER_SIZE);
                checksum = calculate_checksum(checksum, working_buffer,
                                              header->value_size % NVDM_BUFFER_SIZE);
        }
...
static int32_t search_data_item_by_name(const char *group_name,
                                        const char *data_item_name, uint32_t *hasename)
{
        int32_t i, len;
        ...
        if (len < 0) {
                nvdm_port_log_error("group_name=%s, data_item_name=%s", group_name,
                                    data_item_name);
                return 0;
        }
        ...
        for (i = 0; i < g_total_data_item_count; i++) {
                ...
                if (hash == g_data_item_headers[i].hash_name) {
                        peb_read_data(g_data_item_headers[i].pnum,
                                      g_data_item_headers[i].offset + DATA_ITEM_HEADER_SIZE, (uint8_t *)str,
                                      g_data_item_headers[i].group_name_size);
                        if (strcmp(str, group_name)) {
                                continue;
                        }
                        peb_read_data(g_data_item_headers[i].pnum,
                                      g_data_item_headers[i].offset + DATA_ITEM_HEADER_SIZE +
                                      g_data_item_headers[i].group_name_size, (uint8_t *)str,
                                      g_data_item_headers[i].data_item_name_size);
                                  ...
                                  uint32_t *size)
{
        ...
        offset = g_data_item_headers[index].offset + DATA_ITEM_HEADER_SIZE +
        ...
        peb_read_data(g_data_item_headers[index].pnum, offset, (uint8_t *)&checksum2,
        ...
        offset = g_data_item_headers[index].offset + DATA_ITEM_HEADER_SIZE +
                 g_data_item_headers[index].group_name_size +
                 g_data_item_headers[index].data_item_name_size;
        peb_read_data(g_data_item_headers[index].pnum, offset, buffer,
                      g_data_item_headers[index].value_size);
        ...
        *size = g_data_item_headers[index].value_size;
        ...
        nvdm_port_log_info("group_name = %s, data_item_name = %s, size = %d",
                           group_name, data_item_name, g_data_item_headers[index].value_size);
                                   ...
                                   uint32_t size)
{
        ...
        if (append) {
                added_size = size + group_name_size + data_item_name_size +
                             DATA_ITEM_HEADER_SIZE + DATA_ITEM_CHECKSUM_SIZE;
        } else {
        ...
        alloc_size = size + group_name_size + data_item_name_size +
                     DATA_ITEM_HEADER_SIZE + DATA_ITEM_CHECKSUM_SIZE;
        ...
        nvdm_port_log_info("group_name = %s, data_item_name = %s, size = %d",
                           group_name, data_item_name, size);

        /* calculate checksum for new data item copy */
        checksum = 0;
        /* DATA_ITEM_HEADER_SIZE-1 must be power of 2 */
        checksum = calculate_checksum(checksum, &p_data_item_header->pnum,
                                      DATA_ITEM_HEADER_SIZE - 1);
        ...
        working_buffer -= group_name_size;
        checksum = calculate_checksum(checksum, working_buffer,
                                      (group_name_size + data_item_name_size));
        ...
        offset += 1;
        peb_write_data(pnum, offset, &p_data_item_header->pnum,
                       DATA_ITEM_HEADER_SIZE - 1);
        /* write group name and data item name */
        offset += DATA_ITEM_HEADER_SIZE - 1;
        peb_write_data(pnum, offset, working_buffer,
                       group_name_size + data_item_name_size);
        ...
        offset -= p_data_item_header->value_size + DATA_ITEM_HEADER_SIZE +
                  group_name_size + data_item_name_size;
        ...
        if (!append) {
                ...
                peb_read_data(old_pnum, old_offset, (uint8_t *)&data_item_header,
...
nvdm_status_t nvdm_delete_data_item(const char *group_name,
                                    const char *data_item_name)
{
        int32_t index;
...
nvdm_status_t nvdm_delete_group(const char *group_name)
{
        ...
        for (index = 0; index < g_total_data_item_count; index++) {
                ...
                peb_read_data(g_data_item_headers[index].pnum,
                              g_data_item_headers[index].offset + DATA_ITEM_HEADER_SIZE, (uint8_t *)str,
                              g_data_item_headers[index].group_name_size);
...
nvdm_status_t nvdm_query_begin(void)
{
        ...
        malloc_size = (2 * g_sum_data_item_headers * sizeof(uint32_t) + 1) +
                      (g_total_data_item_count / 8 + 1);
        ...
        g_order_data_item_name_table = g_group_name_table + g_sum_data_item_headers +
                                       1; /* need one more */
        search_bitmask = (uint8_t *)(g_order_data_item_name_table +
                                     g_sum_data_item_headers);
        ...
        while (curr_order_index < g_sum_data_item_headers) {
                ...
                for (i = 0; i < g_total_data_item_count; i++) {
                        ...
                        if (is_frist_group_name == true) {
                                ...
                                peb_read_data(g_data_item_headers[i].pnum,
                                              g_data_item_headers[i].offset + DATA_ITEM_HEADER_SIZE, (uint8_t *)search_str,
                                              g_data_item_headers[i].group_name_size);
                                ...
                                peb_read_data(g_data_item_headers[i].pnum,
                                              g_data_item_headers[i].offset + DATA_ITEM_HEADER_SIZE, (uint8_t *)compare_str,
                                              g_data_item_headers[i].group_name_size);
...
nvdm_status_t nvdm_query_next_group_name(char *group_name)
{
        ...
        peb_read_data(data_item_headers->pnum,
                      data_item_headers->offset + DATA_ITEM_HEADER_SIZE, (uint8_t *)group_name,
                      data_item_headers->group_name_size);
...
nvdm_status_t nvdm_query_next_data_item_name(char *data_item_name)
{
        ...
        if (g_curr_data_item_name_index >= (g_group_name_table[g_curr_group_name_index]
                                            - g_group_name_table[g_curr_group_name_index - 1])) {
                return NVDM_STATUS_ITEM_NOT_FOUND;
        }
        ...
        /* check whether group name is match */
        peb_read_data(data_item_headers->pnum,
                      data_item_headers->offset + DATA_ITEM_HEADER_SIZE +
                      data_item_headers->group_name_size,
...
void data_item_scan(int32_t pnum)
{
        ...
        while (offset < (g_nvdm_peb_size - PEB_HEADER_SIZE - DATA_ITEM_HEADER_SIZE)) {
                peb_read_data(pnum, offset, (uint8_t *)&data_item_header,
                              DATA_ITEM_HEADER_SIZE);
                data_item_header_print_info(&data_item_header);
                size = DATA_ITEM_HEADER_SIZE + data_item_header.group_name_size +
                       data_item_header.data_item_name_size + data_item_header.value_size +
                       DATA_ITEM_CHECKSUM_SIZE;
                ...
                case DATA_ITEM_STATUS_WRITING:
                        /* we can't belive data item header if we found it's writting,
                                  ...
                                  */
                        peb_add_drity(pnum, (g_nvdm_peb_size - PEB_HEADER_SIZE) - peb_drity -
                                      peb_valid);
                        return;
                ...
                checksum1 = calculate_data_item_checksum(&data_item_header, pnum,
                                data_item_header.offset + DATA_ITEM_HEADER_SIZE);
                offset += DATA_ITEM_HEADER_SIZE + data_item_header.group_name_size +
                          data_item_header.data_item_name_size + data_item_header.value_size;
                ...
                if (g_data_item_headers[data_item_header.index].sequence_number ==
                                MAX_WRITE_SEQUENCE_NUMBER) {
                        /* we find this frist time */
                        memcpy(&g_data_item_headers[data_item_header.index], &data_item_header,
                        ...
                        if ((abnormal_data_item > 0) ||
                                        (g_data_item_headers[data_item_header.index].sequence_number ==
                                         data_item_header.sequence_number)) {
...
void data_item_init(void)
{
        ...
        g_total_data_item_count = nvdm_port_get_data_item_config(&g_max_data_item_size,
                                  &g_max_group_name_size, &g_max_data_item_name_size);
        ...
        g_data_item_headers = (data_item_header_t *)nvdm_port_malloc(
                                      g_total_data_item_count * sizeof(data_item_header_t));
        if (g_data_item_headers == NULL) {
                nvdm_port_log_error("alloc data_item_headers fail");
                return;
        }

        memset(g_data_item_headers, 0,
               g_total_data_item_count * sizeof(data_item_header_t));
        for (i = 0; i < g_total_data_item_count; i++) {
                           ...
                           int32_t dst_pnum, int32_t dst_offset, int32_t size)
{
        ...
        if (size % NVDM_BUFFER_SIZE) {
                memset(working_buffer, 0, NVDM_BUFFER_SIZE);
                peb_read_data(src_pnum, src_offset + delta, working_buffer,
                              size % NVDM_BUFFER_SIZE);
                peb_write_data(dst_pnum, dst_offset + delta, working_buffer,
                               size % NVDM_BUFFER_SIZE);
        }
}
...
int32_t date_item_migration(int32_t src_pnum, int32_t dst_pnum, int32_t offset)
{
        ...
        while (pos < (g_nvdm_peb_size - PEB_HEADER_SIZE - DATA_ITEM_HEADER_SIZE)) {
                peb_read_data(src_pnum, pos, (uint8_t *)&data_item_header,
                              DATA_ITEM_HEADER_SIZE);
                switch (data_item_header.status) {
                case DATA_ITEM_STATUS_WRITING:
                case DATA_ITEM_STATUS_EMPTY:
                ...
                case DATA_ITEM_STATUS_VALID:
                        if (g_data_item_headers[data_item_header.index].sequence_number !=
                                ...
                                checksum = calculate_data_item_checksum(
                                                   &g_data_item_headers[data_item_header.index], src_pnum,
                                                   data_item_header.offset + DATA_ITEM_HEADER_SIZE);
                                ...
                                peb_write_data(dst_pnum, offset + 1,
                                               &g_data_item_headers[data_item_header.index].pnum, DATA_ITEM_HEADER_SIZE - 1);

                                /* write group name, data item name and value of data item */
                                data_migration(src_pnum, pos + DATA_ITEM_HEADER_SIZE, dst_pnum,
                                               offset + DATA_ITEM_HEADER_SIZE,
                                               data_item_header.group_name_size + data_item_header.data_item_name_size +
                                               data_item_header.value_size);

                                /* write checksum of data item */
                                peb_write_data(dst_pnum,
                                               offset + DATA_ITEM_HEADER_SIZE + data_item_header.group_name_size +
                                               data_item_header.data_item_name_size + data_item_header.value_size,
                                               (uint8_t *)&checksum, DATA_ITEM_CHECKSUM_SIZE);
