...
void peb_write_data(int32_t pnum, int32_t offset, const uint8_t *buf,
                    int32_t len)
{
        int32_t ret;
...
static void relocate_pebs(int32_t *source_pebs, int32_t source_peb_count)
{
        ...
        for (i = 0; i < source_peb_count; i++) {
                total_valid_data += (g_nvdm_peb_size - PEB_HEADER_SIZE) -
                                    g_pebs_info[source_pebs[i]].dirty - g_pebs_info[source_pebs[i]].free;
        }
...
static void garbage_reclaim_pebs(int32_t found_blocks, int32_t *peb_list)
{
        ...
        do {
                ...
                for (i = merge_start; i < found_blocks; i++) {
                        sum_valid += (g_nvdm_peb_size - PEB_HEADER_SIZE) -
                                     g_pebs_info[peb_list[i]].dirty - g_pebs_info[peb_list[i]].free;
...
static void garbage_collection_peb(void)
{
        ...
        for (i = 0; i < g_nvdm_peb_count; i++) {
                ...
                cur_valid = (g_nvdm_peb_size - PEB_HEADER_SIZE) - g_pebs_info[i].dirty -
                            g_pebs_info[i].free;
                if (cur_valid == 0) {
        ...
        for (i = 0; i < g_nvdm_peb_count; i++) {
                ...
                if (g_pebs_info[i].erase_count < (mean_erase_count * (100 -
                                                  PEB_ERASE_COUNT_CRITERION)) / 100) {
                        peb_list[found_blocks++] = i;
                }
        }
        ...
        for (i = 0; i < found_blocks; i++) {
                cur_valid = (g_nvdm_peb_size - PEB_HEADER_SIZE) - g_pebs_info[peb_list[i]].dirty
                            - g_pebs_info[peb_list[i]].free;
                max = i;
                for (j = i; j < found_blocks; j++) {
                        tmp_valid = (g_nvdm_peb_size - PEB_HEADER_SIZE) - g_pebs_info[peb_list[j]].dirty
                                    - g_pebs_info[peb_list[j]].free;
...
int32_t space_allocation(int32_t alloc_size, int32_t added_size,
                         int32_t *poffset)
{
...
static uint32_t calculate_total_avail_space(void)
{
        ...
        criteria2 = (g_nvdm_peb_count - NVDM_RESERVED_PEB_COUNT) *
                    (g_nvdm_peb_size - PEB_HEADER_SIZE - g_max_data_item_size -
                     DATA_ITEM_HEADER_SIZE - DATA_ITEM_CHECKSUM_SIZE - g_max_group_name_size -
                     g_max_data_item_name_size - 2);
...
static void peb_scan(void)
{
                ...
                case PEB_STATUS_RECLAIMING:
                        ...
                        if (peb_status == PEB_STATUS_TRANSFERING) {
                                if (transfering_peb >= 0) {
                                        nvdm_port_log_error("find more than one transfering peb, frist=%d, second=%d",
                                                            transfering_peb, i);
                                        nvdm_port_free(reclaiming_peb);
                                        return;
                                }
                                ...
                                if (transfered_peb >= 0) {
                                        nvdm_port_log_error("find more than one transfered peb, frist=%d, second=%d",
                                                            transfered_peb, i);
                                        nvdm_port_free(reclaiming_peb);
                                        return;
                                }
        ...
        g_valid_data_size = 0;
        for (i = 0; i < g_nvdm_peb_count; i++) {
                g_valid_data_size += (g_nvdm_peb_size - PEB_HEADER_SIZE) - g_pebs_info[i].free -
                                     g_pebs_info[i].dirty;
        }
}
