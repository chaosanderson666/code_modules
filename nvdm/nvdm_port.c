...
#define DEBUG_NVMD_ENABLE   0

#if (DEBUG_NVMD_ENABLE == 1)
...
void nvdm_port_log_notice(const char *message, ...)
{
        ...
        va_start(ap, message);
        vsprintf(sprint_buf, message, ap);
        va_end(ap);
        printf("\n\r%s", sprint_buf);
}
...
void nvdm_port_flash_read(uint32_t address, uint8_t *buffer, uint32_t length)
{
        ...
        status = flash_stream_read(&flash, address, length, buffer);
        if (status != 1) {
                nvdm_port_log_error("hal_flash_read: address = 0x%08x, buffer = 0x%08x, length = %d, status = %d",
                                    address, (uint32_t)buffer, length, status);
        }
}

void nvdm_port_flash_write(uint32_t address, const uint8_t *buffer,
                           uint32_t length)
{
        int status;

        status = flash_stream_write(&flash, address, length, buffer);
        if (status != 1) {
                nvdm_port_log_error("hal_flash_write: address = 0x%08x, buffer = 0x%08x, length = %d, status = %d",
                                    address, (uint32_t)buffer, length, status);
        }
}
...
#define NVDM_PORT_DAT_ITEM_COUNT (500) /*Ruijuan modify to 500. This defines the max item number we can store in NVDM. */
